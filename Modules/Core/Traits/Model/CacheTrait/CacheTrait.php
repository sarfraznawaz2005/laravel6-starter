...
trait CacheTrait
{
     ...
     * @return void
     */
    protected function storeCacheKey($model, $cacheKey)
    {
        $keysFile = storage_path('framework/cache/cacheable.json');
     ...
     * @return array
     */
    protected function flushCacheKeys($model)
    {
        $flushedKeys = [];
        $keysFile = storage_path('framework/cache/cacheable.json');
        $cacheKeys = $this->getCacheKeys($keysFile);
    ...
    public function cacheQuery(Builder $builder, array $columns, Closure $closure)
    {
        ...
        if (method_exists($this->getContainer('cache')->getStore(), 'tags')) {
            $result = $lifetime === -1 ? $this->getContainer('cache')->tags($model)->rememberForever($cacheKey,
                $closure) : $this->getContainer('cache')->tags($model)->remember($cacheKey, $lifetime, $closure);

            return $result;
        }

        $result = $lifetime === -1 ? $this->getContainer('cache')->rememberForever($cacheKey,
            $closure) : $this->getContainer('cache')->remember($cacheKey, $lifetime, $closure);
